...
def orth_para(N, iL):
    ...
    for iv in range(num_v):
        v_param = mx.gluon.Parameter(name='v_for_iL%d_iV%d'%(iL, iv), shape=(N, 1), init=mx.initializer.Normal())
        v_param.initialize(ctx=CTX)
        v = v_param.data(ctx=CTX)
        vv = mx.nd.broadcast_div(
            mx.nd.dot(v, mx.nd.transpose(v)),
            mx.nd.dot(mx.nd.transpose(v), v))
        H = eye(N) - 2*vv
        W = mx.nd.dot(H, W)
    return W
'''
